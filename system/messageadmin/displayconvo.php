...
if($processQuery->num_rows > 0)
{
	while ($resultQuery = $processQuery-> fetch_assoc())
	{
		if ($resultQuery['receiverId'] == $senderId)
		{
			echo '<tr>
				<td>
				<div class="pull-left" style="margin-right:5px;"><img src="../../img/admin.png" style="height:40px;width:40px;border-radius:50px;"></div>
				<div class="pull-left">
					...
					<p style="font-size:14px;">'.$resultQuery['message'].'<br><font size="1">'.$resultQuery['dateTime'].'</font></p>
					</div>
				</div></td></tr>
				';
		}
       ...
       -moz-box-shadow: 0 10px 6px -6px #777;
            box-shadow: 0 10px 6px -6px #777;">
						<p style="font-size:14px;color:white">'.$resultQuery['message'].'<br><font size="1">'.$resultQuery['dateTime'].'</font></p>
						</div>
					</div>
				</div></td></tr>
				';
