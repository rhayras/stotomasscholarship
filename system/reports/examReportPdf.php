...
if($sem == "All")
{
	$pdf->Cell(160, 20, "All Scholars SY ".$schoolyear,'','C');
}
else
{
...
if($examtype == "All")
{
	if($sem == "All")
	{
		$getStudents = "SELECT * FROM tbl_examevaluation WHERE schoolyear = '".$schoolyear."'";
	}
	else
	{
		$getStudents = "SELECT * FROM tbl_examevaluation WHERE schoolyear = '".$schoolyear."' AND semester = '".$sem."' ";
	}
}
else
{
	if($sem == "All")
	{
		$getStudents = "SELECT * FROM tbl_examevaluation WHERE studenttype = '".$examtype."' AND schoolyear = '".$schoolyear."'";
	}
	else
	{
...
if($processStudents->num_rows > 0)
{
	while($resultStudents = $processStudents->fetch_assoc())
	{
		...
		$pdf->Cell(150, 15, $resultInfo['address']." STB",1,'TBLR','L');
